...
CREATE TABLE IF NOT EXISTS `config_system_configuration`(
  ...
  `key` varchar(191) NOT NULL COMMENT 'system config key',
  `value` varchar(1024) DEFAULT NULL COMMENT 'system config value',
  `application` varchar(64) NOT NULL DEFAULT 'odc' COMMENT 'application name',
  `profile` varchar(64) NOT NULL DEFAULT 'default' COMMENT 'profile name',
  `label` varchar(64) NOT NULL DEFAULT 'master' COMMENT 'label name',
